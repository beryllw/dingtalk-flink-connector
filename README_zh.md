...
| `sink.batch.max-size` | Int | 否 | 每批最大记录数，默认 `20` |
| `sink.max-in-flight-requests` | Int | 否 | 最大并发异步请求数，默认 `1` |
| `sink.max-buffered-requests` | Int | 否 | 缓冲区最大请求数，默认 `100` |
| `sink.buffer.flush-interval` | Long | 否 | 缓冲区刷新间隔（毫秒），默认 `5000` |
| `sink.buffer.max-size-in-bytes` | Long | 否 | 缓冲区最大字节数，默认 `5242880`（5MB） |
...
详细使用说明请参阅 [使用手册](docs/USER_GUIDE_zh.md)。
